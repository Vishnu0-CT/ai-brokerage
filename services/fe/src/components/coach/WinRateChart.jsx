...
function WinRateChart({ data, title, subtitle }) {
            ...
            <Bar dataKey="winRate" radius={[4, 4, 0, 0]}>
              {data.map((entry, index) => (
                <Cell
                  ...
                  fillOpacity={entry.isCurrentSlot ? 1 : 0.8}
                  stroke={entry.isCurrentSlot ? '#38BDF8' : 'none'}
                  strokeWidth={entry.isCurrentSlot ? 2 : 0}
                />
              ))}
            </Bar>
          </BarChart>
    ...
    <div className="bg-navy-700 border border-navy-600 rounded-lg px-3 py-2 shadow-lg">
      ...
      {data.pnl !== undefined && (
        <div className={`text-xs font-mono mt-1 ${data.pnl >= 0 ? 'text-positive' : 'text-negative'}`}>
          ...
          {data.currentSessionPnl !== undefined && (
            <span className="text-slate-500"> (incl. {data.currentSessionPnl >= 0 ? '+' : ''}\u20b9{data.currentSessionPnl.toLocaleString('en-IN')} live)</span>
          )}
        </div>
      )}
    </div>
